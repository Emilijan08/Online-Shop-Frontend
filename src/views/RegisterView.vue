  ...
  <div>
      ...
      <div class="bg-gray-700 p-8 rounded-lg shadow-lg max-w-md">
        ...
        <form @submit.prevent="store.register(username, password, confirmPassword)">
          ...
          <div class="mb-6">
            ...
            <input
              class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
              type="password" id="confirm-password" v-model="confirmPassword" required />
          </div>
          <button
            class="bg-white hover:bg-gray-200 text-gray-800 font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
            type="submit">
            Register
          </button>
        </form>
        <p class="mt-4 text-white">
          ...
          <RouterLink to="/login" class="underline hover:text-gray-400">Login here</RouterLink>
        </p>
      </div>
    </div>
    <div :class="store.error ? 'scale-100' : ''" class="fixed scale-0 z-10 inset-0 overflow-y-auto">
      <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center">
        ...
        <!-- Modal content -->
        <div :class="store.error ? 'scale-100' : ''"
          ...
          (password = ''),
          (confirmPassword = '')
          " to="/register">Try again</RouterLink>
            </button>
          </div>
        </div>
      </div>
